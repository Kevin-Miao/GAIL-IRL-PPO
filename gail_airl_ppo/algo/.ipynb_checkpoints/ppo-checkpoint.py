...
from gail_airl_ppo.network import CNNStateIndependentPolicy, CNNStateFunction
...
class PPO(Algorithm):
    ...
    def __init__(self, state_shape, action_shape, device, seed, gamma=0.995,
                 ...
                 max_grad_norm=10.0):
        super().__init__(state_shape[0], action_shape, device, seed, gamma)
...
#         pdb.set_trace()        
    ...
    def step(self, env, state, t, step):
        ...
        mask = False if t == 400000 else done

        self.buffer.append(state[0], action, reward, mask, log_pi, next_state[0])
    ...
    def update_actor(self, states, actions, log_pis_old, gaes, writer):
        log_pis = self.actor.evaluate_log_pi(states, actions[:, 0])
